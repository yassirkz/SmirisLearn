  ...
  return (
    ...
    >
      ...
      <div className="relative">
        {/* Cercle de fond */}
        <div
          className={`absolute inset-0 ${variantClass} bg-gradient-to-r ${colorClass} opacity-20 blur-xl`}
        />
            ...
            >
              {showIcon && (
                ...
                >
                  {CustomIcon ? (
                    <CustomIcon
                      className={`w-${parseInt(size) * 0.6} h-${parseInt(size) * 0.6} text-transparent bg-clip-text bg-gradient-to-r ${colorClass}`}
                    />
                  ) : (
                    <Loader2
                      className={`w-${parseInt(size) * 0.6} h-${parseInt(size) * 0.6} text-transparent bg-clip-text bg-gradient-to-r ${colorClass}`}
  ...
  return (
    <div className="flex flex-col items-center space-y-4">
      <div className="relative">
        ...
        >
          <span
            className={`text-sm font-bold bg-clip-text text-transparent bg-gradient-to-r ${colors[color]}`}
          >
            {progress}%
          </span>
        </motion.div>
